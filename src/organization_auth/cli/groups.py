...
from organization_auth.service_layer.exceptions import GroupDoesNotExist, RoleDoesNotExist
...
@app.command()
def new(team_id: UUID, name: str, role: DCERoleEnum, group_id: Optional[UUID] = None):
    """Creates a new group in a team"""
    if group_id is None:
        group_id = uuid4()
    group = service.create_group(repo, team_id=team_id, group_id=group_id, name=name, role=role)
    show_group(group)
